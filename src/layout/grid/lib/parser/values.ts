...
import type {GridViewColumnInfo, GridViewColumnPattern} from '../../Grid.types';
...
export function templateColumns(tpl: string): GridViewColumnInfo {
  if (!tpl || tpl === 'none') {
    return {
  ...
  const columnPattern = (format: string): GridViewColumnPattern => {
    let fr = 0;
    const sizes: number[] = [];
